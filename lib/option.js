...
function parseOptionSetupString ( str ) {
  ...
  for ( var i=0; i < tokens.length; i++ ) {
    ...
    // assign alias
    if ( !properties.alias && _token.type === 'alias' ) {
      properties.alias = _token.name;
    }
    
    // assign name
    if ( !properties.name && _token.type !== 'alias' ) {
      properties.name = _token.name;
    }
    
    // assign type
    if ( !properties.type ) {
      // option
      if ( /option|require|alias/.test( _token.type ) ) {
        properties.type = 'option';
      // keyword
      } else if ( _token.type === 'keyword' ){
        properties.type = 'keyword';
      }
    }
    
    // assign params
    if ( /required|optional/.test( _token.type ) ) {
      ...
      var option = {
        required: /required/.test( _token.type )
      };
      
      // assign arg options
      if ( _token.options ) {
        option.options = _token.options;
      // or just a name
      } else {
        option.name = _token.name;
      }
  ...
  if ( !properties.name ) {
    ...
    properties.name = props.map( function ( a ) {
      return a.name;
    })[0];
  }
